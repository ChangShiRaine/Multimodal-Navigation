...
class AgentBoxesWithFadedHistory(AgentRepresentation):
    ...
    # def extract_relevant_trajectory(self, instance_token: str, sample_token: str) -> np.ndarray:
    #     """
    #     Extract trajectories of other objects in the input representation.
    #     :param instance_token: Instance token.
    #     :param sample_token: Sample token.
    #     :return: coordinates center_agent_trajectory, other_objects_trajectory
    #     """
    #     center_agent_annotation = self.helper.get_sample_annotation(instance_token, sample_token)
    #     center_agent_yaw = quaternion_yaw(Quaternion(center_agent_annotation['rotation']))
    #     rotation_mat = get_rotation_matrix((0, 0, 0), - center_agent_yaw)
    #     reverse_rotation_mat = get_rotation_matrix((0, 0, 0), center_agent_yaw)
    #
    #     # add 1 to each point to consider the translation factor in rotation matrix
    #     points = np.array([[-self.meters_left, self.meters_ahead, 1],     # up left
    #                        [-self.meters_left, -self.meters_behind, 1],   # down left
    #                        [self.meters_right, -self.meters_behind, 1],     # down right
    #                        [self.meters_right, self.meters_ahead, 1]])  # up right
    #
    #     global_coordinates = reverse_rotation_mat.dot(points.T).T + np.array(center_agent_annotation['translation'][:2])
    #     global_square = Polygon(global_coordinates)
    #
    #     history = self.helper.get_past_for_sample(sample_token,
    #                                               self.seconds_of_history,
    #                                               in_agent_frame=False,
    ...
    #     history = add_present_time_to_history(present_time, history)
    #     # print(history)
    #     relevant_trajectory = dict()
    #     for instance, annotations in history.items():
    #         if len(annotations) != 0:
    #             sequence = []
    #             for n in range(len(annotations)):
    #                 coordinates = annotations[n]['translation'][:2]
    #                 if global_square.contains(Point(coordinates)):
    #                     velocity = self.helper.get_velocity_for_agent(instance, annotations[n]['sample_token'])
    #                     acceleration = self.helper.get_acceleration_for_agent(instance, annotations[n]['sample_token'])
    #                     heading_change_rate = self.helper.get_heading_change_rate_for_agent(instance, annotations[n]['sample_token'])
    #                     # transfer back to the agent coordination system
    #                     local_coordinates = rotation_mat.dot(np.array([center_agent_annotation['translation'][0] - coordinates[0],
    #                                                                    center_agent_annotation['translation'][1] - coordinates[1],
    #                                                                    1]).T)
    #                     # state = np.array(
    #                     #     [coordinates[0], coordinates[1], velocity, acceleration, heading_change_rate])
    #                     state = np.array([local_coordinates[0], local_coordinates[1], velocity, acceleration, heading_change_rate])
    #                     sequence.append(state)
    #
    #             if sequence != []:
    #                 relevant_trajectory[instance] = sequence
    #
    #     center_agent_trajectory = {instance_token : relevant_trajectory.pop(instance_token, None)}
    #     other_objects_trajectory = relevant_trajectory
    #     return center_agent_trajectory, other_objects_trajectory
    ...
    def extract_relevant_trajectory(self, instance_token: str, sample_token: str) -> np.ndarray:
        ...
        relevant_instance = []
        for i in range(len(present_time)):
            coordinates = present_time[i]['translation'][:2]
            if global_square.contains(Point(coordinates)) and (present_time[i]['category_name'] in ['vehicle.car', 'vehicle.truck', 'vehicle.trailer', 'vehicle.bus.rigid', 'vehicle.bus.bendy', 'vehicle.construction', 'vehicle.emergency.police']):
                relevant_instance.append(present_time[i])

        instance_dict = {}
        for item in relevant_instance:
            past_sequence = self.helper.get_past_for_agent(item['instance_token'], item['sample_token'], seconds=self.seconds_of_history, in_agent_frame=False, just_xy=False)[::-1]
            past_sequence.append(item)
            past_state = []
            for r in past_sequence:
                coordinates = r['translation'][:2]
                velocity = self.helper.get_velocity_for_agent(r['instance_token'], r['sample_token'])
                acceleration = self.helper.get_acceleration_for_agent(r['instance_token'], r['sample_token'])
                heading_change_rate = self.helper.get_heading_change_rate_for_agent(r['instance_token'], r['sample_token'])
            ...
            future_sequence = self.helper.get_future_for_agent(item['instance_token'], item['sample_token'], seconds=6, in_agent_frame=False, just_xy=False)
            future_state = []
            for r in future_sequence:
                coordinates = r['translation'][:2]
                velocity = self.helper.get_velocity_for_agent(r['instance_token'], r['sample_token'])
                acceleration = self.helper.get_acceleration_for_agent(r['instance_token'], r['sample_token'])
                heading_change_rate = self.helper.get_heading_change_rate_for_agent(r['instance_token'],
                                                                                    r['sample_token'])

                # transfer back to the agent coordination system
                local_coordinates = rotation_mat.dot(
                    np.array([center_agent_annotation['translation'][0] - coordinates[0],
                              center_agent_annotation['translation'][1] - coordinates[1],
                              1]).T)

                state = np.array(
                    [local_coordinates[0], local_coordinates[1], velocity, acceleration, heading_change_rate])
                future_state.append(state)

            instance_dict[item['instance_token']] = [past_state, future_state]

        center_agent_trajectory = {instance_token : instance_dict.pop(instance_token, None)}
        other_objects_trajectory = instance_dict
        return center_agent_trajectory, other_objects_trajectory