...
class MultimodalGenerativeCVAE(object):
    ...
    def create_node_models(self):
        ...
        self.add_submodule(self.node_type + '/node_future_encoder/initial_h',
                           model_if_absent=DataParallel(nn.Linear(self.state_length,
        ...
        if self.hyperparams['incl_robot_node']:
            ...
            self.add_submodule('robot_future_encoder/initial_h',
                               model_if_absent=DataParallel(nn.Linear(self.robot_state_length,
                                                         self.hyperparams['enc_rnn_dim_future'])))
        ...
        if self.hyperparams['edge_encoding']:
            ...
            if self.hyperparams['edge_influence_combine_method'] == 'bi-rnn':
                ...
                self.add_submodule(self.node_type + '/edge_influence_encoder',
                                   model_if_absent=DataParallel(AdditiveAttention(
        ...
        if self.hyperparams['use_map_encoding']:
            if self.node_type in self.hyperparams['map_encoder']:
                ...
                self.add_submodule(self.node_type + '/map_encoder',
                                   model_if_absent=DataParallel(CNNMapEncoder(me_params['map_channels'],
        ...
        if True:
            if True:
                image_feature_vector_size = 32 
                self.add_submodule(self.node_type + '/image_sequence_encoder',
                                    model_if_absent=DataParallel(ImageSeqEncoder(image_feature_vector_size = image_feature_vector_size )))
        ...
        if True:
            #              Image Sequence encoder
            x_size += image_feature_vector_size #* (self.max_hl + 1)
        ...
        if self.hyperparams['p_z_x_MLP_dims'] is not None:
            self.add_submodule(self.node_type + '/p_z_x',
                               model_if_absent=DataParallel(nn.Linear(x_size, self.hyperparams['p_z_x_MLP_dims'])))
            hx_size = self.hyperparams['p_z_x_MLP_dims']
        else:
            hx_size = x_size

        self.add_submodule(self.node_type + '/hx_to_z',
                           model_if_absent=DataParallel(nn.Linear(hx_size, self.latent.z_dim)))

        if self.hyperparams['q_z_xy_MLP_dims'] is not None:
            self.add_submodule(self.node_type + '/q_z_xy',
                               #                                           Node Future Encoder
                               model_if_absent=DataParallel(nn.Linear(x_size + 4 * self.hyperparams['enc_rnn_dim_future'],
        ...
        self.add_submodule(self.node_type + '/hxy_to_z',
                           model_if_absent=DataParallel(nn.Linear(hxy_size, self.latent.z_dim)))
        ...
        self.add_submodule(self.node_type + '/decoder/state_action',
                           model_if_absent=DataParallel(nn.Sequential(
        ...
        self.add_submodule(self.node_type + '/decoder/initial_h',
                           model_if_absent=DataParallel(nn.Linear(z_size + x_size, self.hyperparams['dec_rnn_dim'])))

        ###################
        #   Decoder GMM   #
        ###################
        self.add_submodule(self.node_type + '/decoder/proj_to_GMM_log_pis',
                           model_if_absent=DataParallel(nn.Linear(self.hyperparams['dec_rnn_dim'],
    ...
    def obtain_encoded_tensors(self,
        ...
        :param image_seqs: Tensor of Image Sequences. [bs, num_images, channels, x, y]
        ...
        ###########################
        if True:
            encoded_image_seq = self.node_modules[self.node_type + '/image_sequence_encoder'](image_seqs)
        ...
        if True:
            x_concat_list.append(encoded_image_seq)
