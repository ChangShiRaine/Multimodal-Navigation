...
class EnvironmentDataset(object):
    def __init__(self, env, state, pred_state, node_freq_mult, scene_freq_mult, hyperparams, nuscenes_dataset, **kwargs):
        self.env = env
        self.state = state
        ...
        for node_type in env.NodeType:
            ...
            self.node_type_datasets.append(NodeTypeDataset(env, node_type, state, pred_state, node_freq_mult,
                                                           scene_freq_mult, hyperparams, nuscenes_dataset, **kwargs))
...
class NodeTypeDataset(data.Dataset):
    def __init__(self, env, node_type, state, pred_state, node_freq_mult,
                 scene_freq_mult, hyperparams, nuscenes_dataset, augment=False, **kwargs):
        self.nuscenes_dataset = nuscenes_dataset
        self.env = env
        self.state = state
        self.pred_state = pred_state
    ...
    def __getitem__(self, i):
        ...
        return get_node_timestep_data(self.env, scene, t, node, self.state, self.pred_state,
                                      self.edge_types, self.max_ht, self.max_ft, self.hyperparams, self.nuscenes_dataset)
