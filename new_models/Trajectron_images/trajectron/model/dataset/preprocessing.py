...
nu_path = '../../../experiments/nuScenes/devkit/python-sdk/'
...
def get_node_timestep_data(env, scene, t, node, state, pred_state,
                           ...
                           nuscenes_dataset, scene_graph=None):
    """
    ...
    #Front Camera
    front_camera_images = None
    nuScenes_path = '/home/aplesnia/nuScenes/'
    #sample_tokens = scene.scene_sample_tokens[max(t - max_ht, 0): t+1]
    sample_tokens = [scene.scene_sample_tokens[t]]
    filenames = [nuscenes_dataset.get('sample_data', nuscenes_dataset.get('sample', sample_token)['data']['CAM_FRONT'])['filename'] for sample_token in sample_tokens]
    ...
    front_camera_images = [transform(Image.open(nuScenes_path + filename)) for filename in filenames]
    #if len(front_camera_images) < (max_ht + 1):
    #    padding = [torch.zeros(front_camera_images[0].shape)] * ((max_ht + 1) - len(front_camera_images))
    #    front_camera_images = padding + front_camera_images
    front_camera_images = torch.stack(front_camera_images)
    ...
    return (first_history_index, x_t, y_t, x_st_t, y_st_t, neighbors_data_st,
            neighbors_edge_value, robot_traj_st_t, map_tuple, front_camera_images)


def get_timesteps_data(env, scene, t, node_type, state, pred_state,
                       edge_types, min_ht, max_ht, min_ft, max_ft, hyperparams, nuscenes_dataset):
    """
    ...
    for timestep in nodes_per_ts.keys():
            ...
            for node in present_nodes:
                ...
                batch.append(get_node_timestep_data(env, scene, timestep, node, state, pred_state,
                                                    edge_types, max_ht, max_ft, hyperparams, nuscenes_dataset,
