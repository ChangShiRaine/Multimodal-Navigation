...
from model.new_rnn_model import STRNN
...
class MultimodalGenerativeCVAE(object):
    def __init__(self,
        ...
        self.use_strnn = True   #control
        print(self.use_strnn)

        if self.use_strnn:
            self.strnn88 = STRNN(8, 8, 8 ).cuda()
            self.strnn68 = STRNN(6, 8, 8).cuda()
            self.strnn86 = STRNN(8, 6, 8).cuda()
            self.strnn66 = STRNN(6, 6, 8).cuda()
        '''
        if self.use_strnn:
            self.strnn88 = STRNN(8, 8, 8 )
            self.strnn68 = STRNN(6, 8, 8)
            self.strnn86 = STRNN(8, 6, 8)
            self.strnn66 = STRNN(6, 6, 8)
        '''
    ...
    def encode_edge(self,
        ...
        if self.hyperparams['edge_state_combine_method'] == 'sum':
            ...
            combined_neighbors = torch.stack(op_applied_edge_states_list, dim=0) #256 9 (t) 8 (feat)
            ...
            if self.hyperparams['dynamic_edges'] == 'yes':
                ...
                combined_edge_masks = torch.stack(op_applied_edge_mask_list, dim=0)
        if self.use_strnn:
            if edge_states_list[0].size(-1) == 8 and node_history_st.size(-1) == 8:
                joint_history = self.strnn88(edge_states_list, node_history_st)
            elif edge_states_list[0].size(-1) == 8 and node_history_st.size(-1) == 6:
                joint_history = self.strnn86(edge_states_list, node_history_st)
            elif edge_states_list[0].size(-1) == 6 and node_history_st.size(-1) == 8:
                joint_history = self.strnn68(edge_states_list, node_history_st)
            elif edge_states_list[0].size(-1) == 6 and node_history_st.size(-1) == 6:
                joint_history = self.strnn66(edge_states_list, node_history_st)
            else:
                print('error')
        else:
